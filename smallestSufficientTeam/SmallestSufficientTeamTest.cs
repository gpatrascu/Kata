...
public class Solution
{
    ...
    {
        var startingList = people.Select((list, i) => (i, list)).Where(tuple => tuple.list.Count > 0)
            .OrderByDescending(tuple => tuple.list.Count(req_skills.Contains)).ToList();

        return GetSufficientTeam(startingList, req_skills.ToHashSet());
    }

    private static void PrintArray(ITestOutputHelper testOutputHelper, int[] newValue)
    {
        
        testOutputHelper.WriteLine(string.Join(',', newValue));
    }

    private static int[] GetSufficientTeam(IEnumerable<(int i, IList<string> list)> tupleList, HashSet<string> reqSkills)
    {
        var output = new SortedSet<int>();
        foreach (var tuple in tupleList)
        {
            foreach (var skill in tuple.list)
            {
                output.Add(tuple.i);
